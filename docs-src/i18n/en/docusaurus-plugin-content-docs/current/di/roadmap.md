---
displayed_sidebar: injectContainerSidebar
title: Roadmap / Changes
---

## Observações baseadas no código atual

- Há sinais de padronização de nome (cabeçalho menciona InjectContainer e também “Injector4D”).
- Existe um overload comentado para `Singleton<T>` com `ATag` (sugere suporte futuro a tags para classes).

---

## Notes based on current code

- There are naming standardization traces (headers mention InjectContainer and also “Injector4D”).
- There is a commented overload for `Singleton<T>` with `ATag` (suggesting future support for tagging class registrations).


