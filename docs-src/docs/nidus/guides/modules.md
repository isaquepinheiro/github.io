...
title: Módulos (TModule)
---

Módulos são a unidade de composição do Nidus. Um módulo encapsula:

- `Routes`: rotas/módulos filhos associados ao escopo
- `Binds`: injeção de dependência (registro de classes/serviços)
...
:::caution
`ExportedBinds` exporta apenas binds. Rotas não são exportadas/importadas.
:::
